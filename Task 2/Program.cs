...
namespace Task_2
{
    public class Program
    {
        ...
        static decimal NumPlace(int ball, int box)
        {
            ...
            // Перемножаем оставшиеся множители знаменателя
        ...
        private static void Main(string[] args)
        {
            ...
            decimal resultA = 1;        // Результат для красных шаров
            // Если красные шары есть, то определяем кол-во размещений для них
            if (A != 0)
                resultA = NumPlace(A,N);
            
            decimal resultB = 1;        // Результат для синих шаров
            // То же для синих
            if (B != 0)
                resultB = NumPlace(B, N);
